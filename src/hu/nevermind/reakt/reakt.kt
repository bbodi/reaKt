...
abstract public class ReactClass<P, S>(body: ReactElementContainer.() -> Unit = {}) {
    ...
    public fun createElement(): ReactElement {
        ...
        val constructorParams = fullOptions.toJavaScriptObject()
...
private fun <V> Iterable<Pair<String, V>>.toJavaScriptObject(): dynamic {
    val jsObj: dynamic = object{}
    for ((key, value) in this) {
        val convertedValue: Any = when (value) {
            is ReactRef -> value.name
            else -> value
        }
        jsObj[key] = convertedValue
    }
    return jsObj
}
